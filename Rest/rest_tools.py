...
class RestTools:
    ...
    @staticmethod
    async def configure_http_request(request_type: str = 'GET', host_address: str = '127.0.0.1', port: int | str = 5000, resource: str = '/monitoring', headers: dict = None, query: dict = None, data: dict = None, message: str = None, timeout: int = 5):
        '''
        ...
        if message is not None:
            if status == 200:
                log_message(f'Succesful request: {message}', type='SUCCESS')
            else:
    ...
    async def start_crowd_monitoring(request_type: str ='POST', host_address: str ='rest_crowd_host', port: int | str ='rest_crowd_port', resource: str ='/monitoring', message: str ='Crowd stats monitoring started'):
        ...
        _, status_code, reason = await RestTools.configure_http_request(request_type=request_type, host_address=host_address, port=port, resource=resource, message= message)

        return {'Response': status_code, 'Reason': reason}
    

    @staticmethod
    async def stop_crowd_monitoring(request_type: str ='GET', host_address: str ='rest_crowd_host', port: int | str ='rest_crowd_port', resource: str ='/monitoring', message: str ='Crowd stats monitoring stopped', timeout: int = 15):
        '''
        ...
        response_data, status_code, reason = await RestTools.configure_http_request(request_type=request_type, host_address=host_address, port=port, resource=resource, message=message, timeout=timeout)
        ...
        log_message(message=f'{data}', type='HIGHLIGHT')
    ...
    async def start_crowd_cpu_monitoring(request_type: str ='POST', host_address: str ='rest_crowd_host', port: int | str ='rest_crowd_port', resource: str ='/resources/monitor/', message: str = 'Crowd CPU monitoring started', body: dict = None):
        ...
        _, status_code, reason = await RestTools.configure_http_request(request_type=request_type, host_address=host_address, port=port, resource=resource, data=body, message=message)

        return {'Response': status_code, 'Reason': reason}


    @staticmethod
    async def stop_crowd_cpu_monitoring(request_type: str ='GET', host_address: str ='rest_crowd_host', port: int | str ='rest_crowd_port', resource: str ='/resources/monitor/', message: str ='Crowd CPU monitoring stopped', timeout: int = 15):
        '''
        ...
        log_message(message=f'{data}', type='WARNING')
    ...
    async def monitor_crowd():
        ...
        if crowd_response.get('Response') == 200:
            ...
            if crowd_response.get('Response') == 200:
                ...
                response = {'Response': 'Failed', 'Reason': crowd_response.get('Reason')}
            
            response['Status'] = True
        else:
            log_message(message='Crowd monitoring failed', type='ERROR')
            response = {'Response': 'Failed', 'Status': False, 'Reason': crowd_response.get('Reason')}
    ...
    async def terminate_monitoring_crowd():
        ...
        if crowd_stats.get('Crowdcell_stats') is not None:
            ...
            if cpu_stats.get('CPU_stats') is not None:
                ...
                response = {'Response': 'OK', 'Status': True}
            else:
                log_message(message='Crowd CPU monitoring failed', type='ERROR')
                response = {'Response': 'Failed', 'Status': False, 'Reason': 'CPU monitoring returned None'}
                cpu_stats = {'CPU_stats': None}
            
        else:
            log_message(message='Crowd cell monitoring failed', type='ERROR')
            response = {'Response': 'Failed', 'Status': False, 'Reason': 'Crowd monitoring returned None'}
            crowd_stats = {'Crowdcell_stats': None}
            cpu_stats = {'CPU_stats': None}

        # Add the stats to the response
        log_message(message=f'Terminating monitoring response: {response}', type=('ERROR' if response.get('Status') == False else 'SUCCESS'))
